...
cmd({
  ...
  try {
    ...
    if (m.mentionedJid.includes(botNumber)) {
      const thumbnailRes = await axios.get(config.MENU_IMAGE_URL || " https://files.catbox.moe/1r2s5m.png", {
      ...
      await conn.sendMessage(m.chat, {
        ...
        contextInfo: {
          ...
          isForwarded: true,
          externalAdReply: {
            title: config.BOT_NAME || "MASTERTECH-XD V2 🥀",
            body: config.DESCRIPTION || "> *ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴍᴀsᴛᴇʀᴛᴇᴄʜ-xᴅ*",
            mediaType: 1,
            renderLargerThumbnail: true,
            thumbnail: thumbnailBuffer,
            mediaUrl: " https://files.catbox.moe/1r2s5m.png", // Static image URL
